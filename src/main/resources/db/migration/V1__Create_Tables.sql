...
(
    id            bigint auto_increment primary key,
    name          varchar(100)            not null comment '사이트명',
    base_url      varchar(255)            not null comment '기본 도메인',
    is_active     boolean   default true comment '수집 활성화 여부',
    rate_limit_ms bigint    default 60000 comment '요청 간격',
    headers       json comment '공통 헤더 (User-Agent, Cookie 등)',
    timeout       bigint    default 60000 not null,
    created_at    timestamp default current_timestamp
);

create table tbl_crawl_page_definition
(
    id           bigint auto_increment primary key,
    site_id      bigint      not null,
    page_type    varchar(20) not null comment '페이지 역할 (LIST, DETAIL, ...)',
#     url_pattern  varchar(255) not null comment 'URL 매칭 정규식',
    fetch_method varchar(20) default 'JSOUP' comment '수집 방식(JSOUP, SELENIUM)',
    description  varchar(200),
    foreign key (site_id) references tbl_crawl_site (id)
);

create table tbl_crawl_seed
(
    id                 bigint auto_increment primary key,
    site_id            bigint       not null,
    page_definition_id bigint       not null,
    url                varchar(500) not null comment '진입점 URL (예: .../news/101)',
    description        varchar(100) comment '설명 (예: 네이버 뉴스-경제)',
    is_active          boolean               default true,
    interval_seconds   int          not null default 3600 comment '수집 주기(초)',
    next_run_at        bigint       not null default 0 comment '다음 실행 예정시간(timestamp)',
    last_processed_at  bigint       null comment '마지막 실행 시간',
    foreign key (site_id) references tbl_crawl_site (id),
    foreign key (page_definition_id) references tbl_crawl_page_definition (id),
    index idx_crawl_seed_execution (site_id, next_run_at)
);

create table tbl_crawl_selector
(
    id                        bigint auto_increment primary key,
    page_definition_id        bigint not null,
    rule_type                 varchar(20) default 'DATA' comment 'DATA(데이터 추출), LINK(새 작업 발견)',
    target_page_definition_id bigint,
    field_name                varchar(50) comment '필드명(title, price, content, article, next_page_link)',
    selector_type             varchar(20) default 'CSS' comment 'CSS, XPATH, REGEX, JSON_PATH',
    selector_value            text comment '실제 선택자 (div.content > p)',
    inner_selector_value      text,
    extract_attribute         varchar(50) comment '추출할 속성 (text, href, src 등)',
    is_multiple               boolean     default false comment '결과가 리스트인지 여부',
    foreign key (page_definition_id) references tbl_crawl_page_definition (id),
    foreign key (target_page_definition_id) references tbl_crawl_page_definition (id)
);

create table tbl_crawl_queue
(
    id                 bigint auto_increment primary key,
    site_id            bigint       not null,
    ...
    last_processing_at timestamp,
    next_run_at        timestamp   default current_timestamp,
    expired_at         timestamp,
    created_at         timestamp   default current_timestamp,
    updated_at         timestamp   default current_timestamp,
    foreign key (site_id) references tbl_crawl_site (id),
    foreign key (seed_id) references tbl_crawl_seed (id),
    foreign key (page_definition_id) references tbl_crawl_page_definition (id),
    index idx_crawl_queue_dispatch (site_id, status, next_run_at),
    index idx_zombie_queue (expired_at)
);

create table tbl_crawl_result
(
    id             bigint auto_increment primary key,
...
create table tbl_crawl_queue_history
(
